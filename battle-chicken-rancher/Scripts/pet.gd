...
func _physics_process(delta: float) -> void:
	...
	# RESTORED: Console Stats Logging every 5 seconds
	debug_timer += delta
	if debug_timer >= 5.0:
		debug_timer = 0.0
		print("[%s] State: %s | Hunger: %d/%d | Energy: %d/%d | Happiness: %d" % [
			data.species_name, State.keys()[current_state], 
			data.current_hunger, data.max_hunger, data.current_energy, data.max_energy, data.happiness
		])
...
func _update_needs(delta: float) -> void:
	if current_state == State.SLEEPING:
		if data.current_hunger > 25.0:
			data.current_energy += (10.0 * data.rest_multiplier) * delta
			data.accumulated_rest_time += delta
			_check_evolution()
	else:
		data.current_energy -= 1.0 * delta
	...
	# RESTORED: Happiness math!
	if current_state == State.WANDER:
		data.happiness += 0.5 * delta
...
func _evaluate_state_machine() -> void:
	...
	if current_state == State.IDLE and state_timer <= 0:
		...
		elif roll < 0.1 and energy_pct < 80.0: 
			_transition_to(State.FINDING_BED)
		else:
...
func _check_evolution() -> void:
	...
	if data.accumulated_rest_time >= threshold and data.evolution_tier < 4:
		# RESTORED: Evolution notification
